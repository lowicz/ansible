...
def check_blocking_io():
    ...
    for handle in (sys.stdin, sys.stdout, sys.stderr):
        ...
        if not os.get_blocking(fd):
            handles.append(getattr(handle, 'name', None) or f'#{fd}')

    if handles:
        raise SystemExit(
            (
                'ERROR: Ansible requires blocking IO on stdin/stdout/stderr. '
                'Non-blocking file handles detected: %s' % ', '.join(handles)
            )
        )
...
def initialize_locale():
    ...
    try:
        ...
        raise SystemExit(
            f'ERROR: Ansible could not initialize the preferred locale: {e}'
        )


    if not encoding or encoding.lower() not in ('utf-8', 'utf8'):
        raise SystemExit(
            f'ERROR: Ansible requires the locale encoding to be UTF-8; Detected {encoding}.'
        )
    ...
    if fs_enc.lower() != 'utf-8':
        raise SystemExit(
            f'ERROR: Ansible requires the filesystem encoding to be UTF-8; Detected {fs_enc}.'
        )
    ...
    def get_password_from_file(pwd_file):
        ...
        if b_pwd_file == b'-':
            ...
            try:
                ...
                raise AnsibleError(
                    f"Problem occured when trying to run the password script {pwd_file} ({e}). If this is not a script, remove the executable bit from the file."
                )
